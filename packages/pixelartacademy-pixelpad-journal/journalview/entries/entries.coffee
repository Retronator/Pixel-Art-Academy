...
class PAA.PixelPad.Apps.Journal.JournalView.Entries extends AM.Component
  @id: -> 'PixelArtAcademy.PixelPad.Apps.Journal.JournalView.Entries'
  @version: -> '0.1.0'
  ...
  onCreated: ->
    ...
    @entries = new ComputedField =>
      ...
      for entryDocument in entryDocuments
        @_entries[entryDocument._id] ?= new PAA.PixelPad.Apps.Journal.JournalView.Entry @, entryDocument._id
      ...
      if @journalDesign.options.journalId and not @journalDesign.options.readOnly and @firstLoadDone
        entries.push new PAA.PixelPad.Apps.Journal.JournalView.Entry @, null
