...
class PAA.PixelPad.Apps.Journal.JournalsView extends AM.Component
  @id: -> 'PixelArtAcademy.PixelPad.Apps.Journal.JournalsView'
  @version: -> '0.1.0'
  ...
  onRendered: ->
    ...
    @autorun =>
      ...
      pixelPadSize = @journal.os.pixelPad.animatingSize()

      # Update the camera's aspect.
      camera = @camera()
      camera.aspect = pixelPadSize.width / pixelPadSize.height
      camera.updateProjectionMatrix()
    ...
    @autorun (computation) =>
      # Depend on PixelPad size and number of journals.
      width = @journal.os.pixelPad.animatingSize().width
