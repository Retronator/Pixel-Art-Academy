...
class HQ.Checkout extends LOI.Adventure.Location
  @id: -> 'Retronator.HQ.Store.Checkout'
  ...
  things: ->
    [
      PAA.Cast.Retro.id()
      HQ.Checkout.Display.id()
      HQ.Actors.ElevatorButton.id()
    ]

  exits: ->
    exits = @elevatorExits()
    exits[Vocabulary.Keys.Directions.North] = HQ.Steps.id()
    exits[Vocabulary.Keys.Directions.Down] = HQ.Steps.id()
    exits[Vocabulary.Keys.Directions.East] = HQ.Store.id()
    exits
  ...
  onScriptsLoaded: ->
    ...
    Tracker.autorun (computation) =>
      ...
      retroDialog = @scripts['Retronator.HQ.Store.Checkout.Scripts.Retro']
