...
class PixelArtAcademy extends Artificial.Base.App
  ...
  _addPage: (name, url, page) ->
    if LOI.isRunningLocally()
      @_addLocalRoute name, url, 'PixelArtAcademy.Layouts.LocalAccess', page, 'PixelArtAcademy.Pages.Intro'
  ...
  _addLocalRoute: (name, url, layout, page, intro) ->
    FlowRouter.route url,
      name: name
      action: (params, queryParams) ->
        BlazeLayout.render layout,
