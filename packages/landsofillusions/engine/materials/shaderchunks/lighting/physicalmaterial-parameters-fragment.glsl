...
vec3 F_Burley(const in float dotNV, const in float dotNL, const in float dotVH, const in float roughness) {
  ...
  vec3 viewScatter = F_Schlick(vec3(1.0), f90, dotNL);
...
void RE_Direct_Physical(const in IncidentLight directLight, const in GeometricContext geometry, const in PhysicalMaterial material, inout ReflectedLight reflectedLight) {
  ...
  vec3 Fspecular = F_Schlick( material.specularColor, material.specularF90, dotVH );
  vec3 Fdiffuse = F_Burley(dotNV, dotNL, dotVH, material.roughness);
  float V = V_GGX_SmithCorrelated( alpha, dotNL, dotNV );
  float D = D_GGX( alpha, dotNH );

  vec3 irradiance = dotNL * directLight.color;
  reflectedLight.directSpecular += irradiance * Fspecular * V * D;
  reflectedLight.directDiffuse += irradiance * Fdiffuse * BRDF_Lambert(material.diffuseColor);
}
