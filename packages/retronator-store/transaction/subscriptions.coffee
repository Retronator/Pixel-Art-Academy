...
Meteor.publish RS.Transaction.forCurrentUser, ->
  ...
  @autorun =>
    ...
    # twitter handle, or patreon ID, since that's how transactions are found.
    ...
    # We also need to have user's login services, registered emails, and additional
    # autogenerated service fields so we can match transactions on the client.
