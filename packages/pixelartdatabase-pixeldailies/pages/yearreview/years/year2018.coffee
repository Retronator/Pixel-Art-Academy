AM = Artificial.Mirage
PADB = PixelArtDatabase

class PADB.PixelDailies.Pages.YearReview.Years[2018]
  @backgrounds = [
    # This is the year review header image.
    url: 'https://pbs.twimg.com/media/DV7-SSNXkAYEeZ5.png'
    position: ['50%', '10%']
    author: 'Erien27'
  ,
    url: 'https://pbs.twimg.com/media/DYxUnhUVoAAqsxh.jpg'
    position: ['30%', '80%']
    author: 'Hainsaw'
  ,
    url: 'https://pbs.twimg.com/media/DSq0PJeVAAE55Ml.png'
    position: ['0%', '40%']
    author: 'LumpyTouch'
  ,
    url: 'https://pbs.twimg.com/media/DZXg2SpXkAAm43X.jpg'
    position: ['70%', '30%']
    author: 'Zel_D'
  ,
    url: 'https://pbs.twimg.com/tweet_video_thumb/DTK4Re5X0AA-BWa.jpg'
    position: ['0%', '95%']
    author: 'Uknowleo'
  ,
    url: 'https://pbs.twimg.com/media/DYLox7GXcAACNCg.png'
    position: ['30%', '0%']
    author: 'FilipeBrizolara'
  ,
    url: 'https://pbs.twimg.com/media/DUYA7IWXUAA-bpd.jpg'
    position: ['0%', '100%']
    author: 'HermineRenarde'
  ,
    url: 'https://pbs.twimg.com/media/DTo5f3NVoAE71yP.png'
    position: ['70%', '20%']
    author: 'astronautcurve'
  ,
    url: 'https://pbs.twimg.com/media/DYnKi2nW4AEcQJs.png'
    position: ['40%', '70%']
    author: 'WowaTheU'
  ]
