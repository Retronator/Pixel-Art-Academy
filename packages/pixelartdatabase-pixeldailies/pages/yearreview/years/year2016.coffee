AM = Artificial.Mirage
PADB = PixelArtDatabase

class PADB.PixelDailies.Pages.YearReview.Years[2016]
  @backgrounds = [
    # This is the year review header image.
    url: 'https://pbs.twimg.com/media/Ci9SIL8UkAI1tI5.png'
    position: ['0%', '50%']
    author: 'skittlefuck'
  ,
    # This one appears on the about page
    url: 'https://pbs.twimg.com/media/Cecn8oIW8AAK6uS.jpg'
    position: ['0%', '100%']
    author: 'orangemagik'
  ,
    url: 'https://pbs.twimg.com/media/CxLBA7PVEAELURt.png'
    position: ['100%', '0%']
    author: 'jtangc'
  ,
    url: 'https://pbs.twimg.com/tweet_video_thumb/CX6sqnbW8AQ9ECT.png'
    position: ['30%', '40%']
    author: 'Weilard'
  ,
    url: 'https://pbs.twimg.com/media/Cx-y8-sUAAABbl1.png'
    position: ['100%', '0%']
    author: 'skittlefuck'
  ,
    url: 'https://pbs.twimg.com/media/CjW6CwjUYAIQVo8.png'
    position: ['0%', '100%']
    author: 'watawatabou'
  ,
    url: 'https://pbs.twimg.com/media/CvO23_LVUAY5ZwK.png'
    position: ['50%', '0%']
    author: 'ricardojuchem'
  ,
    url: 'https://pbs.twimg.com/media/CzXz-x3UAAEWb4e.jpg'
    position: ['0%', '70%']
    author: 'Helgram'
  ,
    url: 'https://pbs.twimg.com/tweet_video_thumb/CnM1_1uWEAADG9K.jpg'
    position: ['80%', '0%']
    author: 'Weilard'
  ,
    url: 'https://pbs.twimg.com/media/CoK5EvbUkAAOfUh.jpg'
    position: ['100%', '0%']
    author: 'VeryJerryPie'
  ]
